...
cateogyr_list = html('.position>li')
...
for row in cur.fetchall():
	...
	html = JQ.get('%s/product/list.html?cate_no=%s'%(site.site_url,upper_category))
	...
	for category in category_list :
		...
		if category_code is upper_category:
			continue
		category_desc = pq(category.find('a')).text().split('(')[0]
...
for row in pbar(cur.fetchall()):
	...
	page_cnt = 1
	while page_cnt == 1:
		html = JQ.get('%s/product/list.html?cate_no=%s&page=%d'%(site.site_url,category,page_cnt))
		product_list = html('.prdList .prdImg')
		if product_list.length < 1 : #존재하지 않는 페이지일경우 stop
			break
		for product in product_list :
			# product_code = pq(pq(product).find('a')).attr['name'].split('_')[1]

			url = pq(pq(product).find('a')).attr['href']
			product_code = url.split('/')[3]
			...
			html = JQ.get('%s/product/detail.html?product_no=%s'%(site.site_url , product_code))
			product_nm = html('span[name="wp_detection"]').eq(1).text()
			product_desc = html('#subsc').text().replace('\n','').replace('\t','').strip()[:998]

			if product_desc == '' :
				desc_spliter = '상품'
				product_desc = html('.cont').text().encode('utf8').split(desc_spliter)[0].replace('\n','').replace('\t','').replace('.','').decode('utf8').strip()[:998]

			product_price = html('#span_product_price_text').text()
			product_price = ''.join(re.findall('\d+', product_price.encode('utf8') )) if product_price != '' else '0'
			product_img = html('.thumbnail img').attr['src']
			...
			product_img_list = html('#prdDetail img')
